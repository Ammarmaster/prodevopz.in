...
export default async function StudentDashboard({ searchParams }: Props) {
  ...
  const allTasksCompleted = approvedSubmissions.length >= tasks.length && tasks.length > 0;
  const paymentCompleted = student.paymentStatus === "COMPLETED";
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-16 flex flex-col gap-12">
      ...
      <div className="glass-panel border-white/[0.06] p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {allTasksCompleted && paymentCompleted && (
            <>
              <Link
              ...
              <Link
                ...
                target="_blank"
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-accent-purple/30 hover:bg-accent-purple/5 text-xs font-bold text-accent-purple transition-colors"
              >
      ...
      {allTasksCompleted && paymentCompleted && (
        <div className="glass-panel border-emerald-500/20 bg-emerald-500/5 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex gap-3 text-left">
            <CheckCircle className="w-6 h-6 text-emerald-400 shrink-0" />
            <div>
              <h4 className="text-xs font-bold text-white">Graduation Confirmed & Paid</h4>
              <p className="text-[10px] text-foreground/50 leading-relaxed mt-0.5">
                Your payment of ₹200 was processed successfully (Payment ID: {student.paymentId}). Your credentials have been emailed to you from contactprodevopz@gmail.com.
              </p>
            </div>
          </div>
        </div>
